...
{
	IOWR_ALTERA_AVALON_PIO_DATA(KEYCODE4_BASE, keycode);
}

void setKeycode5(WORD keycode)
{
	IOWR_ALTERA_AVALON_PIO_DATA(KEYCODE5_BASE, keycode);
}
int main() {
	BYTE rcode;
	...
	while (1) {
		...
		if (GetUsbTaskState() == USB_STATE_RUNNING) {
			if (!runningdebugflag) {
				...
				setKeycode4(kbdbuf.keycode[3]);
				setKeycode5(kbdbuf.keycode[4]);
