...
filename="/home/pi/lab3/datapath_midterm.csv"
rssi_filename = "/home/pi/lab3/rssi_midterm.csv"
#joy_filename = "/home/pi/lab3/joystick_midterm.csv"
...
for idx, x in enumerate(riemann_result_yaw):
    ...
    if idx > 260 and idx < 380:
        if x < 0:
            riemann_rounded[idx] = (np.pi/2)*(math.floor(x) if x - math.ceil(x) <= -0.3 else math.ceil(x))
        else:
            riemann_rounded[idx] = (np.pi/2)*(math.ceil(x) if x - math.floor(x) >= 0.3 else math.floor(x))
    else:
...
for i in range(len(riemann_result_yaw)):
    if i in peak_array:
        cur_position = cur_position + step_length * angle[:,i]
...
xy_avg = t0[max_pos]
yy_avg = t1[max_pos]
plt.scatter(t0/2,t1/2, c=rssi_values, cmap='viridis', s=100)
#plt.scatter(xy,yy,c='red',marker='x')
plt.scatter(xy_avg/2,yy_avg/2,c='blue',marker='+')
